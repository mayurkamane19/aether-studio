...
async function createKnowledgeArticle({ category = 'GENERAL', title, content, author = 'Admin', isInternal = false }) {
  const countSql = `SELECT COUNT(*) AS count FROM knowledge_articles;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const articleId = `KB-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO knowledge_articles (article_id, category, title, content, status, author, is_internal, created_at, updated_at)
    VALUES ($1, $2, $3, $4, 'PUBLISHED', $5, $6, NOW(), NOW())
    RETURNING id, article_id AS "articleId", title, category, status;
  `;
  return await query(sql, [articleId, category, title, content, author, isInternal]);
}

async function getKnowledgeArticles(includeInternal = false) {
  let sql = `
    SELECT id, article_id AS "articleId", category, title, content, status, author, is_internal AS "isInternal", created_at AS "createdAt"
    FROM knowledge_articles
  `;
  if (!includeInternal) {
    sql += ` WHERE is_internal = false AND status = 'PUBLISHED'`;
  }
  sql += ` ORDER BY created_at DESC;`;
  return await query(sql);
}

async function createAIRecommendation({ leadId = null, module = 'CROSS_MODULE', category = 'ACTION', title, description, actionPayload = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM ai_recommendations;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const recId = `REC-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO ai_recommendations (rec_id, lead_id, module, category, title, description, action_payload, status, created_at)
    VALUES ($1, $2, $3, $4, $5, $6, $7, 'PENDING', NOW())
    RETURNING id, rec_id AS "recId", title, status;
  `;
  return await query(sql, [recId, leadId, module, category, title, description, actionPayload ? JSON.stringify(actionPayload) : null]);
}

async function getAIRecommendations(status = 'PENDING') {
  const sql = `
    SELECT id, rec_id AS "recId", lead_id AS "leadId", module, category, title, description,
           action_payload AS "actionPayload", status, created_at AS "createdAt"
...
async function updateAIRecommendationStatus(recId, status) {
  const sql = `
    UPDATE ai_recommendations
    SET status = $1
    WHERE rec_id = $2
    RETURNING id, rec_id AS "recId", status;
  `;
async function createSavedReport({ name, category = 'EXECUTIVE', queryConfig, owner = 'Admin', visibility = 'ADMIN' }) {
  const countSql = `SELECT COUNT(*) AS count FROM saved_reports;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const reportId = `RPT-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO saved_reports (report_id, name, category, query_config, owner, visibility, created_at, updated_at)
    VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
    RETURNING id, report_id AS "reportId", name, category;
  `;
  return await query(sql, [reportId, name, category, JSON.stringify(queryConfig), owner, visibility]);
}

async function getSavedReports(category = null) {
  let sql = `
    SELECT id, report_id AS "reportId", name, category, query_config AS "queryConfig",
           owner, visibility, created_at AS "createdAt"
    FROM saved_reports
  `;
  const params = [];
  if (category) {
...
async function logSecurityEvent({ eventType, severity = 'MEDIUM', actor = 'System', ipAddress = '127.0.0.1', details = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM security_events;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const eventId = `SEC-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO security_events (event_id, event_type, severity, actor, ip_address, details, created_at)
    VALUES ($1, $2, $3, $4, $5, $6, NOW())
    RETURNING id, event_id AS "eventId", event_type AS "eventType";
  `;
  return await query(sql, [eventId, eventType, severity, actor, ipAddress, details ? JSON.stringify(details) : null]);
}

async function getSecurityEvents(limit = 50) {
  const sql = `
    SELECT id, event_id AS "eventId", event_type AS "eventType", severity, actor,
...
async function saveClientOnboarding({ leadId, companyInfo = null, projectGoals = '', completedSteps = 1, totalSteps = 5 }) {
  const countSql = `SELECT COUNT(*) AS count FROM client_onboarding;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const onboardingId = `ONB-2026-${String(nextNum).padStart(4, '0')}`;

  const status = completedSteps >= totalSteps ? 'COMPLETED' : 'IN_PROGRESS';

  const sql = `
    INSERT INTO client_onboarding (onboarding_id, lead_id, company_info, project_goals, completed_steps, total_steps, status, created_at, updated_at)
    VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
    ON CONFLICT (onboarding_id) DO UPDATE
    SET company_info = EXCLUDED.company_info, project_goals = EXCLUDED.project_goals,
        completed_steps = EXCLUDED.completed_steps, status = EXCLUDED.status, updated_at = NOW()
    RETURNING id, onboarding_id AS "onboardingId", status, completed_steps AS "completedSteps";
  `;
  return await query(sql, [onboardingId, leadId, companyInfo ? JSON.stringify(companyInfo) : null, projectGoals, completedSteps, totalSteps, status]);
}

async function getClientOnboarding(leadId) {
  const sql = `
    SELECT id, onboarding_id AS "onboardingId", lead_id AS "leadId", company_info AS "companyInfo",
           project_goals AS "projectGoals", completed_steps AS "completedSteps", total_steps AS "totalSteps",
...
async function createMarketingExperiment({ name, variantA, variantB }) {
  const countSql = `SELECT COUNT(*) AS count FROM marketing_experiments;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const expId = `EXP-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO marketing_experiments (exp_id, name, variant_a, variant_b, conversion_a, conversion_b, status, created_at)
    VALUES ($1, $2, $3, $4, 0, 0, 'ACTIVE', NOW())
    RETURNING id, exp_id AS "expId", name, status;
  `;
  return await query(sql, [expId, name, variantA, variantB]);
}

async function getMarketingExperiments() {
  const sql = `
    SELECT id, exp_id AS "expId", name, variant_a AS "variantA", variant_b AS "variantB",
           conversion_a AS "conversionA", conversion_b AS "conversionB", status, created_at AS "createdAt"
...
async function createAIAutomationRule({ name, triggerEvent, actionType, requiresApproval = true }) {
  const countSql = `SELECT COUNT(*) AS count FROM ai_automation_rules;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const ruleId = `ARUL-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO ai_automation_rules (rule_id, name, trigger_event, action_type, requires_approval, status, created_at)
    VALUES ($1, $2, $3, $4, $5, 'ACTIVE', NOW())
    RETURNING id, rule_id AS "ruleId", name, status;
  `;
  return await query(sql, [ruleId, name, triggerEvent, actionType, requiresApproval]);
}

async function getAIAutomationRules() {
  const sql = `
    SELECT id, rule_id AS "ruleId", name, trigger_event AS "triggerEvent", action_type AS "actionType",
           requires_approval AS "requiresApproval", status, created_at AS "createdAt"
...
async function createKPITarget({ kpiName, targetValue, period = 'MONTHLY', owner = 'Executive' }) {
  const countSql = `SELECT COUNT(*) AS count FROM kpi_targets;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const targetId = `TGT-2026-${String(nextNum).padStart(4, '0')}`;
  const val = parseFloat(targetValue) || 0.00;

  const sql = `
    INSERT INTO kpi_targets (target_id, kpi_name, target_value, period, owner, status, created_at)
    VALUES ($1, $2, $3, $4, $5, 'ACTIVE', NOW())
    RETURNING id, target_id AS "targetId", kpi_name AS "kpiName", target_value AS "targetValue", status;
  `;
  return await query(sql, [targetId, kpiName, val, period, owner]);
}

async function getKPITargets() {
  const sql = `
    SELECT id, target_id AS "targetId", kpi_name AS "kpiName", target_value AS "targetValue",
           period, owner, status, created_at AS "createdAt"
    FROM kpi_targets
    ORDER BY created_at DESC;
  `;
  return await query(sql);
...
async function logWorkflowExecution({ workflowId, triggerEvent, status = 'SUCCESS', errorDetails = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM workflow_execution_logs;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const executionId = `EXEC-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO workflow_execution_logs (execution_id, workflow_id, trigger_event, status, error_details, executed_at)
    VALUES ($1, $2, $3, $4, $5, NOW())
    RETURNING id, execution_id AS "executionId", status;
  `;
  return await query(sql, [executionId, workflowId, triggerEvent, status, errorDetails]);
}

async function getWorkflowExecutionLogs(workflowId = null) {
  let sql = `
    SELECT id, execution_id AS "executionId", workflow_id AS "workflowId", trigger_event AS "triggerEvent",
           status, error_details AS "errorDetails", executed_at AS "executedAt"
  ...
  sql += ` ORDER BY executed_at DESC LIMIT 50;`;
  return await query(sql, params);
}

async function createWorkspaceMessage({ channel = 'GENERAL', sender = 'Admin', content, isPinned = false }) {
  const countSql = `SELECT COUNT(*) AS count FROM workspace_messages;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const messageId = `WMSG-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO workspace_messages (message_id, channel, sender, content, is_pinned, created_at)
    VALUES ($1, $2, $3, $4, $5, NOW())
    RETURNING id, message_id AS "messageId", channel, sender, content;
  `;
  return await query(sql, [messageId, channel, sender, content, isPinned]);
}

async function getWorkspaceMessages(channel = 'GENERAL') {
  const sql = `
    SELECT id, message_id AS "messageId", channel, sender, content, is_pinned AS "isPinned", created_at AS "createdAt"
    FROM workspace_messages
    WHERE channel = $1
    ORDER BY created_at ASC;
  `;
  return await query(sql, [channel]);
}

async function logWebhookEvent({ provider, eventType, signature = null, payload = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM webhook_logs;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const webhookId = `WHK-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO webhook_logs (webhook_id, provider, event_type, signature, status, payload, created_at)
    VALUES ($1, $2, $3, $4, 'PROCESSED', $5, NOW())
    RETURNING id, webhook_id AS "webhookId", provider, status;
  `;
  return await query(sql, [webhookId, provider, eventType, signature, payload ? JSON.stringify(payload) : null]);
}

async function getWebhookLogs(provider = null) {
  let sql = `
    SELECT id, webhook_id AS "webhookId", provider, event_type AS "eventType",
           signature, status, created_at AS "createdAt"
    FROM webhook_logs
  `;
  const params = [];
  if (provider) {
...
async function createSecurityIncident({ title, severity = 'MEDIUM', owner = 'Security Team', details = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM security_incidents;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const incidentId = `INC-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO security_incidents (incident_id, title, severity, owner, status, details, created_at)
    VALUES ($1, $2, $3, $4, 'OPEN', $5, NOW())
    RETURNING id, incident_id AS "incidentId", title, severity, status;
  `;
  return await query(sql, [incidentId, title, severity, owner, details ? JSON.stringify(details) : null]);
}

async function getSecurityIncidents() {
  const sql = `
    SELECT id, incident_id AS "incidentId", title, severity, owner, status,
           details, created_at AS "createdAt", resolved_at AS "resolvedAt"
...
async function createRevenueGoal({ period = 'MONTHLY', targetAmount, currentAmount = 0.00 }) {
  const countSql = `SELECT COUNT(*) AS count FROM revenue_goals;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const goalId = `GOAL-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO revenue_goals (goal_id, period, target_amount, current_amount, created_at)
    VALUES ($1, $2, $3, $4, NOW())
    RETURNING id, goal_id AS "goalId", period, target_amount AS "targetAmount", current_amount AS "currentAmount";
  `;
  return await query(sql, [goalId, period, targetAmount, currentAmount]);
}

async function getRevenueGoals() {
  const sql = `
    SELECT id, goal_id AS "goalId", period, target_amount AS "targetAmount",
           current_amount AS "currentAmount", created_at AS "createdAt"
    FROM revenue_goals
    ORDER BY created_at DESC;
  `;
  return await query(sql);
...
async function logAgentAction({ agentType, toolName, status = 'PROPOSED', details = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM agent_logs;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const actionId = `ACT-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO agent_logs (action_id, agent_type, tool_name, status, details, created_at)
    VALUES ($1, $2, $3, $4, $5, NOW())
    RETURNING id, action_id AS "actionId", agent_type AS "agentType", tool_name AS "toolName", status;
  `;
  return await query(sql, [actionId, agentType, toolName, status, details ? JSON.stringify(details) : null]);
}

async function getAgentLogs(agentType = null) {
  let sql = `
    SELECT id, action_id AS "actionId", agent_type AS "agentType", tool_name AS "toolName",
           status, details, created_at AS "createdAt"
    FROM agent_logs
  `;
  const params = [];
  if (agentType) {
...
async function logClientAIQuery({ leadId, queryText, responseText, confidenceLevel = 'HIGH_CONFIDENCE' }) {
  const countSql = `SELECT COUNT(*) AS count FROM client_ai_queries;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const queryId = `CAQ-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO client_ai_queries (query_id, lead_id, query_text, response_text, confidence_level, created_at)
    VALUES ($1, $2, $3, $4, $5, NOW())
    RETURNING id, query_id AS "queryId", lead_id AS "leadId", confidence_level AS "confidenceLevel";
  `;
  return await query(sql, [queryId, leadId, queryText, responseText, confidenceLevel]);
}

async function getClientAIQueries(leadId) {
  const sql = `
    SELECT id, query_id AS "queryId", lead_id AS "leadId", query_text AS "queryText",
           response_text AS "responseText", confidence_level AS "confidenceLevel", created_at AS "createdAt"
    FROM client_ai_queries
    WHERE lead_id = $1
...
async function createPredictiveAlert({ category = 'REVENUE', severity = 'MEDIUM', title, evidence = null, recommendation = null }) {
  const countSql = `SELECT COUNT(*) AS count FROM predictive_alerts;`;
  const countRes = await query(countSql);
  const nextNum = (countRes.success && countRes.rows[0] ? parseInt(countRes.rows[0].count, 10) : 0) + 1;
  const alertId = `PAL-2026-${String(nextNum).padStart(4, '0')}`;

  const sql = `
    INSERT INTO predictive_alerts (alert_id, category, severity, title, evidence, recommendation, status, created_at)
    VALUES ($1, $2, $3, $4, $5, $6, 'OPEN', NOW())
    RETURNING id, alert_id AS "alertId", category, severity, title, status;
  `;
  return await query(sql, [alertId, category, severity, title, evidence, recommendation]);
}

async function getPredictiveAlerts(category = null) {
  let sql = `
    SELECT id, alert_id AS "alertId", category, severity, title, evidence,
           recommendation, status, created_at AS "createdAt"
